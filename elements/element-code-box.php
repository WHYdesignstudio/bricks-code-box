...
if ( class_exists('\Bricks\Element') && ! class_exists('BCB_Element_Code_Box') ) {
  class BCB_Element_Code_Box extends \Bricks\Element {
    ...
    public function enqueue_scripts() {
      $theme = isset($this->settings['theme']) ? preg_replace('/[^a-z0-9\-]/i','',$this->settings['theme']) : 'prism';
      $theme_url = 'https://cdn.jsdelivr.net/npm/prismjs/themes/' . $theme . '.min.css';
      wp_enqueue_style('bcb-prism-theme', $theme_url, [], null);
    ...
    public function render() {
      ...
      echo '<div ' . $this->render_attributes('_root') . '>';
        if ( $showCopy ) {
          ...
          $initialLabel = $is_en ? ($settings['label_copy_en'] ?? '📋 Copy') : ($settings['label_copy_de'] ?? '📋 Kopieren');
